...
export default function UserManagementClient({ initialUsers }: Props) {
  ...
  const [formData, setFormData] = useState<UserPatchRequestDTO>({
    name: "",
    email: "",
    role: "",
  });
          ...
          <form onSubmit={handleUpdate} className="space-y-4">
            ...
            <div className="flex gap-3 justify-end pt-2">
              <button
                type="button"
                onClick={handleCancelEdit}
                className="px-4 py-2 text-zinc-600 dark:text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-700 rounded-lg transition-colors font-medium"
              >
                Cancelar
              </button>
              <button
              ...
              {filteredUsers.length === 0 ? (
                ...
                filteredUsers.map((user) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold border ${ROLE_STYLES[user.role] || "bg-zinc-100 text-zinc-800"}`}
                      >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-zinc-800 rounded-lg p-6 max-w-md w-full shadow-xl">
            ...
            <p className="text-zinc-600 dark:text-zinc-400 mb-6">
              ...
              <strong>{userToDelete.name}</strong>? Esta ação removerá o acesso
              dele à plataforma.
            </p>
            <div className="flex justify-end gap-3">
              <button
                ...
                className="px-4 py-2 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700 rounded-lg transition-colors font-medium"
              >
                Cancelar
              </button>
              <button
