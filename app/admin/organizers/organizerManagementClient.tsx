...
export default function OrganizerManagementClient({
  initialOrganizers,
}: Props) {
  ...
  const [organizerToDelete, setOrganizerToDelete] =
    useState<OrganizerResponseDTO | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (isEditing) {
        ...
        setOrganizers((prev) =>
          prev.map((o) => (o.id === isEditing ? { ...o, ...formData } : o)),
        );
        setIsEditing(null);
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setOrganizers((prev) =>
        prev.filter((o) => o.id !== organizerToDelete.id),
      );
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-zinc-800 rounded-lg p-6 max-w-md w-full shadow-xl">
            ...
            <p className="text-zinc-600 dark:text-zinc-400 mb-6">
              ...
              <strong>{organizerToDelete?.name}</strong>? Esta ação não pode ser
              desfeita.
            </p>
